...
void ATEMstd::delay(const unsigned int delayTimeMillis)
{ // Responsible delay function which keeps the ATEM run loop up! DO NOT USE INSIDE THIS CLASS! Recursion could happen...
...
{
	return (atemTallyByIndexTallyFlags[inputNumber - 1] & 1) ? true : false;
}
boolean ATEMstd::getPreviewTally(uint8_t inputNumber)
{
	return (atemTallyByIndexTallyFlags[inputNumber - 1] & 2) ? true : false;
}
boolean ATEMstd::getUpstreamKeyerStatus(uint8_t inputNumber)
{
	return getKeyerOnAirEnabled(0, inputNumber - 1);
}
boolean ATEMstd::getUpstreamKeyerOnNextTransitionStatus(uint8_t inputNumber)
{ // input 0 = background
...
void ATEMstd::changeTransitionPositionDone()
{ // When the last value of the transition is sent (1000), send this one too (we are done, change tally lights and preview bus!)
...
{ // Supporting "Background" by "0"
	if (keyer >= 0 && keyer <= 4)
	{ // Todo: Should match available keyers depending on model?
...
void ATEMstd::changeDVESettingsTemp(unsigned long Xpos, unsigned long Ypos, unsigned long Xsize, unsigned long Ysize)
{ // TEMP
	commandBundleStart();
...
void ATEMstd::changeDVEMaskTemp(unsigned long top, unsigned long bottom, unsigned long left, unsigned long right)
{	// TEMP
	// N/A
}
void ATEMstd::changeDVEBorder(bool enableBorder)
{ // TEMP
	setKeyDVEBorderEnabled(0, 0, enableBorder);
}

void ATEMstd::changeDVESettingsTemp_Rate(uint8_t rateFrames)
{ // TEMP
...
void ATEMstd::_parseGetCommands(const char *cmdStr)
{
	...
	// log commands for statistics
	// Serial.printf("%.4s\n", cmdStr);

	// Ignore very frequent commands
	if (!strcmp_P(cmdStr, PSTR("TrPs")))
		return;
	else if (!strcmp_P(cmdStr, PSTR("Time")))
		return;
	else if (!strcmp_P(cmdStr, PSTR("MPrp")))
		return;
	else if (!strcmp_P(cmdStr, PSTR("FMTl")))
		return;
	...
	if (!strcmp_P(cmdStr, PSTR("TlIn")))
	{
		...
		if (atemTallyCallback != NULL)
		{
			atemTallyCallback(&atemTallyProgram, &atemTallyPreview);
		}
	}
	else if (!strcmp_P(cmdStr, PSTR("FASP")))
		return;
	else if (!strcmp_P(cmdStr, PSTR("AEBP")))
		return;
	...
	else if (!strcmp_P(cmdStr, PSTR("TrSS")))
	{
		...
		if (mE <= 1)
		{
			...
			{
				Serial.print(F("atemTransitionStyle[mE="));
				Serial.print(mE);
				Serial.print(F("] = "));
	...
	else if (!strcmp_P(cmdStr, PSTR("TMxP")))
	{
		...
		if (mE <= 1)
		{
			...
			{
				Serial.print(F("atemTransitionMixRate[mE="));
				Serial.print(mE);
				Serial.print(F("] = "));
	...
	else if (!strcmp_P(cmdStr, PSTR("DskP")))
	{
		...
		if (keyer <= 1)
		{
			...
			{
				Serial.print(F("atemDownstreamKeyerPreMultiplied[keyer="));
				Serial.print(keyer);
				Serial.print(F("] = "));
			...
			{
				Serial.print(F("atemDownstreamKeyerInvertKey[keyer="));
				Serial.print(keyer);
				Serial.print(F("] = "));
	...
	else if (!strcmp_P(cmdStr, PSTR("MPCE")))
	{
		...
		if (mediaPlayer <= 1)
		{
			...
			{
				Serial.print(F("atemMediaPlayerSourceType[mediaPlayer="));
				Serial.print(mediaPlayer);
				Serial.print(F("] = "));
			...
			{
				Serial.print(F("atemMediaPlayerSourceStillIndex[mediaPlayer="));
				Serial.print(mediaPlayer);
				Serial.print(F("] = "));
			...
			{
				Serial.print(F("atemMediaPlayerSourceClipIndex[mediaPlayer="));
				Serial.print(mediaPlayer);
				Serial.print(F("] = "));
	...
	else if (!strcmp_P(cmdStr, PSTR("_ver")))
	{
...
/**
 * @brief Set callback when TlIn packet comes
 *
 * @param cb
 */
void ATEMstd::setAtemTallyCallback(atem_tally_cb_t cb)
{
	atemTallyCallback = cb;
}
